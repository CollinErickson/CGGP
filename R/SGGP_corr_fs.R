...
SGGP_internal_CorrMatCauchySQ <- function(x1, x2,theta, return_dCdtheta = FALSE, return_numpara =FALSE,returnlogs = FALSE) {
  if(return_numpara){
    ...
    if(return_dCdtheta){
      if(!returnlogs){
        dCdtheta = cbind(3*expHE*((1+halpha)^(pow-1))*(halpha),0*3*C*pow*log(1+halpha))
      }else{
        dCdtheta = cbind(3*expHE*halpha/(1+halpha),0*3*C)
      }
      dCdtheta[is.na(dCdtheta)] = 0
      out <- list(C=C,dCdtheta=dCdtheta)
      return(out)
...
SGGP_internal_CorrMatGaussian <- function(x1, x2,theta, return_dCdtheta = FALSE, return_numpara =FALSE) {
  if(return_numpara){
    return(1);
  }else{ 
...
SGGP_internal_CorrMatMatern52 <- function(x1, x2,theta, return_dCdtheta = FALSE, return_numpara =FALSE) {
  if(return_numpara){
    return(1);
  }else{ 
    ...
    if(return_dCdtheta){
      dCdtheta <- ((sqrt(5)*diffmat+10/3*diffmat*h)*exp(-sqrt(5)*h) - sqrt(5)*C*diffmat) * (-3/expLS)
      dCdtheta[is.na(dCdtheta)] = 0
      out <- list(C=C,dCdtheta=dCdtheta)
      return(out)
    }else{
...
SGGP_internal_CorrMatPowerExp <- function(x1, x2,theta, return_dCdtheta = FALSE, return_numpara =FALSE) {
  if(return_numpara){
    ...
    if(return_dCdtheta){
      dCdtheta <- (1-nug)*cbind(tmax*alpha*C*diffmat^alpha/expLS^alpha, -C*h^alpha*log(h)/2 * (maxpower - minpower))
      dCdtheta[is.na(dCdtheta)] = 0
      out <- list(C=C,dCdtheta=dCdtheta)
      return(out)
    }else{
