...
neglogpost <- function(theta,SG, y) {
  ...
  if (max(theta) >= 0.9999 || min(theta) <= -0.9999) {
    ...
    if(!is.matrix(y)){
      neglogpost = 1/2*(length(y)*log(sigma_hat[1])-3*sum(log(1-theta)+log(theta+1)-theta^2)+lDet)
    }else{
      neglogpost = 1/2*(dim(y)[1]*sum(log(c(sigma_hat)))-3*sum(log(1-theta)+log(theta+1)-theta^2)+dim(y)[2]*lDet)
    }
...
calc_gneglogpost <- function(theta, SG, y, return_lik=FALSE) {
  ...
  if(!is.matrix(y)){
    neglogpost = 1/2*(length(y)*log(sigma_hat[1])-3*sum(log(1-theta)+log(theta+1)-theta^2)+lDet)
    gneglogpost = 3*(1/(1-theta)-1/(theta+1)+2*theta)+ dlDet+ length(y)*dsigma_hat / sigma_hat[1]
  }else{
    neglogpost = 1/2*(dim(y)[1]*sum(log(c(sigma_hat)))-3*sum(log(1-theta)+log(theta+1)-theta^2)+dim(y)[2]*lDet)
    gneglogpost = 3*(1/(1-theta)-1/(theta+1)+2*theta)+dim(y)[2]*dlDet
    for(i in 1:dim(y)[2]){
      gneglogpost = gneglogpost + dim(y)[1]*dsigma_hat[,i] / sigma_hat[i]
    }
  }
