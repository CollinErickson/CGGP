...
CGGP_internal_neglogpost <- function(theta,CGGP,y,...,ys=NULL,Xs=NULL,HandlingSuppData = "Correct") {
  ...
  if (max(theta) >= 1 || min(theta) <= -1) {
    ...
    if(HandlingSuppData == "Only" || 
      ...
      for (dimlcv in 1:CGGP$d) { # Loop over dimensions
        V = CGGP$CorrMat(Xs[,dimlcv], Xs[,dimlcv], theta[(dimlcv-1)*CGGP$numpara+1:CGGP$numpara],returnlogs=TRUE)
        Sigma_t = Sigma_t+V
      }
      Sigma_t = exp(Sigma_t)
    ...
    if(HandlingSuppData == "Ignore"){
      sigma2anddsigma2 <- CGGP_internal_calcsigma2(CGGP=CGGP, y=y, theta=theta, return_lS=TRUE)
    ...
    if(HandlingSuppData == "Correct"){
      lik_stuff <- CGGP_internal_faststuff1(CGGP=CGGP, y=y, theta=theta)
      ...
      for (dimlcv in 1:CGGP$d) { # Loop over dimensions
        V = CGGP$CorrMat(Xs[,dimlcv], CGGP$xb[1:CGGP$sizest[max(CGGP$uo[,dimlcv])]],theta[(dimlcv-1)*CGGP$numpara+1:CGGP$numpara],returnlogs=TRUE)
        GGGG[[dimlcv]] = exp(V)
    ...
    if(HandlingSuppData == "Correct" ){
      ...
      rcpp_fastmatclcr(CGGP$uo[1:CGGP$uoCOUNT,], CGGP$w[1:CGGP$uoCOUNT], MSE_s,Sigma_t2,CGGP$maxlevel)
