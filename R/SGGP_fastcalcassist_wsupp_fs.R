...
SGGP_internal_calcpwwsupp<- function(SGGP, y, theta, return_lS=FALSE) {
...
SGGP_internal_calcpwanddpwwsupp<- function(SGGP, y, theta, return_lS=FALSE) {
...
SGGP_internal_calcsigma2wsupp <- function(SGGP, y, theta, return_lS=FALSE) {
  Q  = max(SGGP$uo[1:SGGP$uoCOUNT,]) # Max level of all blocks
  cholS = list(matrix(1,1,1),Q*SGGP$d) # To store choleskys
  if(return_lS){
    lS = matrix(0, nrow = max(SGGP$uo[1:SGGP$uoCOUNT,]), ncol = SGGP$d) # Save log determinant of matrices
