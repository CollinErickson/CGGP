...
CGGP_internal_gneglogpost <- function(theta, CGGP, y,..., return_lik=FALSE,ys=NULL,Xs=NULL,HandlingSuppData = "Correct") {
  ...
  if(HandlingSuppData == "Only" || 
    ...
    for (dimlcv in 1:CGGP$d) { # Loop over dimensions
      V = CGGP$CorrMat(Xs[,dimlcv], Xs[,dimlcv], theta[(dimlcv-1)*CGGP$numpara+1:CGGP$numpara],return_dCdtheta=TRUE,returnlogs=TRUE)
      dSigma_to[[dimlcv]] = V$dCdtheta
      Sigma_t = Sigma_t+V$C
    }
    Sigma_t = exp(Sigma_t)
    
    Cmat1 = matrix( rep(Sigma_t , CGGP$numpara ) , nrow = nrow(Sigma_t) , byrow = FALSE )
    for (dimlcv in 1:CGGP$d) { # Loop over dimensions
      ...
      for(paralcv in 1:CGGP$numpara){
        ...
        if(is.matrix(dsigma2_hat_supp )){
          if(dim(dsigma2_hat_supp)[1]>1.5){
            dsigma2_hat_supp[(dimlcv-1)*CGGP$numpara+paralcv,] = -colSums(as.matrix(tempvec1*tempvec2))/dim(Xs)[1]
          }else{
            dsigma2_hat_supp[,(dimlcv-1)*CGGP$numpara+paralcv] = -colSums(as.matrix(tempvec1*tempvec2))/dim(Xs)[1]
          }
        }else{
        ...
        dlDet_supp[(dimlcv-1)*CGGP$numpara+paralcv] = sum(diag(backsolve(Sigma_chol,backsolve(Sigma_chol,dSigma_supp,transpose = TRUE))))
      }
    }
  ...
  if(HandlingSuppData == "Ignore" ||
    ...
    sigma2anddsigma2 <- CGGP_internal_calcsigma2anddsigma2(CGGP=CGGP, y=y, theta=theta, return_lS=TRUE)
  ...
  if(HandlingSuppData == "Correct"){
   ...
   # dCs = array(0,dim=c(dim(Xs)[1],CGGP$numpara*CGGP$ss,dim(Xs)[2]))
    ...
    for (dimlcv in 1:CGGP$d) { # Loop over dimensions
      ...
      V = CGGP$CorrMat(Xs[,dimlcv],  Xbn , theta[(dimlcv-1)*CGGP$numpara+1:CGGP$numpara],return_dCdtheta=TRUE,returnlogs=TRUE)
      A = round(as.vector(outer(CGGP$designindex[,dimlcv],length(Xbn)*(0:(CGGP$numpara-1)),FUN=function(x,y) x+y)))
      GGGG[[dimlcv]] = exp(V$C)
    ...
    for (dimlcv in 1:CGGP$d) { # Loop over dimensions
      Cmat2 = matrix(rep(GGGG[[dimlcv]], CGGP$numpara ) ,nrow = nrow(GGGG[[dimlcv]]) , byrow = FALSE )
  ...
  if(HandlingSuppData == "Correct"){
    MSE_s = matrix(NaN,nrow=dim(Xs)[1]*dim(Xs)[1],ncol=(CGGP$d)*(CGGP$maxlevel))
    dMSE_s = matrix(NaN,nrow=dim(Xs)[1]*dim(Xs)[1],ncol=CGGP$numpara*CGGP$d*CGGP$maxlevel)
    ...
    for (dimlcv in 1:CGGP$d) {
      ...
      for (levellcv in 1:max(CGGP$uo[1:CGGP$uoCOUNT,dimlcv])) {
        ...
        REEALL = CGGP_internal_postvarmatcalcfaster(TT1,
                                                    TT2,
                                                    cholS[[gg+levellcv]],
                                                    dSV[[gg+levellcv]],
                                                    INDSN,
        ...
        for(paralcv in 1:CGGP$numpara){
          dMSE_s[,CGGP$numpara*(dimlcv-1)*CGGP$maxlevel+(paralcv-1)*CGGP$maxlevel+levellcv]  =  as.vector(REEALL$dSigma_mat[,((dim(Xs)[1]*(paralcv-1)+1):(dim(Xs)[1]*paralcv))])
        }
      }
    ...
    for (dimlcv in 1:CGGP$d) {
      for(paralcv in 1:CGGP$numpara){
        dSigma_to2[,CGGP$numpara*(dimlcv-1)+paralcv] = as.vector(t((dSigma_to[[dimlcv]])[,((dim(Xs)[1]*(paralcv-1)+1):(dim(Xs)[1]*paralcv))]))
      }
    }
    
   rcpp_fastmatclcranddclcr(CGGP$uo[1:CGGP$uoCOUNT,], CGGP$w[1:CGGP$uoCOUNT], MSE_s,dMSE_s, Sigma_t2,dSigma_to2,CGGP$maxlevel,CGGP$numpara)
    
   for (dimlcv in 1:CGGP$d) {
     dSigma_to[[dimlcv]] = matrix(dSigma_to2[,CGGP$numpara*(dimlcv-1)+(1:CGGP$numpara)],nrow=dim(Xs)[1] , byrow = FALSE)
   }
   Sigma_t = matrix(Sigma_t2,nrow=dim(Xs)[1] , byrow = FALSE)
    ...
    for (dimlcv in 1:CGGP$d) {
      for(paralcv in 1:CGGP$numpara){
        dCpn = as.matrix((dCs[[dimlcv]])[,((paralcv-1)*dim(Cs)[2]+1):(paralcv*dim(Cs)[2])])
        if(is.matrix(dsigma2_hat_part2)){
          if(dim(dsigma2_hat_part2)[1]>1.5){
            dsigma2_hat_part2[(dimlcv-1)*CGGP$numpara+paralcv,] = -2*colSums((tempvec1)*(dCpn%*%pw))
          }else{
            dsigma2_hat_part2[,(dimlcv-1)*CGGP$numpara+paralcv] =  -2*colSums((tempvec1)*(dCpn%*%pw))
          }
        }else{
        ...
        if(is.matrix(dsigma2_hat_part2)){
          if(dim(dsigma2_hat_part2)[1]>1.5){
            dsigma2_hat_part1[(dimlcv-1)*CGGP$numpara+paralcv,] = -colSums(tempvec1*tempvec2)
          }else{
            dsigma2_hat_part1[,(dimlcv-1)*CGGP$numpara+paralcv] = -colSums(tempvec1*tempvec2)
          }
        }else{
        ...
        dlDet_supp[(dimlcv-1)*CGGP$numpara+paralcv] = sum(diag(backsolve(Sigma_chol,backsolve(Sigma_chol,dSigma_now,transpose = TRUE))))
      }
    }
    ...
    dsigma2_hat_part3 =  -2*(CGGP_internal_faststuff3(CGGP,y,temp4,cholS,dSV)$dvalo)
    lDet_supp = 2*sum(log(diag(Sigma_chol)))
  ...
  if(HandlingSuppData =="Ignore"){
    ...
    if(!is.matrix(y)){
      neglogpost = neglogpost+1/2*(dim(CGGP$design)[1]*log(sigma2_hat[1])-0.500*sum(log(1-theta)+log(theta+1))+lDet)
      gneglogpost = gneglogpost+0.25*(1/(1-theta)-1/(theta+1))+ 1/2*dlDet+ 1/2*length(y)*dsigma2_hat / sigma2_hat[1]
    }else{
      neglogpost = neglogpost+1/2*(dim(CGGP$design)[1]*sum(log(c(sigma2_hat)))-0.500*sum(log(1-theta)+log(theta+1))+dim(y)[2]*lDet)
      gneglogpostn = 0.5*(1/(1-theta)-1/(theta+1))+dim(y)[2]*dlDet
      for(i in 1:dim(y)[2]){
        gneglogpostn = gneglogpostn + dim(CGGP$design)[1]*dsigma2_hat[,i] / sigma2_hat[i]
      }
      gneglogpost = gneglogpost+  gneglogpostn/2
    }
  }
  ...
  if(HandlingSuppData =="Correct"){
    sigma2_hat = sigma2_hat_grid*dim(CGGP$design)[1]/(dim(Xs)[1]+dim(CGGP$design)[1])+sigma2_hat_supp*dim(Xs)[1]/(dim(Xs)[1]+dim(CGGP$design)[1])
    dsigma2_hat = dsigma2_hat_grid*dim(CGGP$design)[1]/(dim(Xs)[1]+dim(CGGP$design)[1])+dsigma2_hat_supp*dim(Xs)[1]/(dim(Xs)[1]+dim(CGGP$design)[1])
    dlDet = dlDet_grid+dlDet_supp
    lDet = lDet_grid+lDet_supp
    
    if(!is.matrix(y)){
      neglogpost = 1/2*((dim(CGGP$design)[1]+dim(Xs)[1])*log(sigma2_hat[1])-0.500*sum(log(1-theta)+log(theta+1))+lDet)
      gneglogpost = 0.25*(1/(1-theta)-1/(theta+1))+ 1/2*dlDet+ 1/2*(length(y)+length(ys))*dsigma2_hat / sigma2_hat[1]
    }else{
      neglogpost = 1/2*((dim(CGGP$design)[1]+dim(Xs)[1])*sum(log(c(sigma2_hat)))-0.500*sum(log(1-theta)+log(theta+1))+dim(y)[2]*lDet)
      gneglogpost = 0.5*(1/(1-theta)-1/(theta+1))+dim(y)[2]*dlDet
      for(i in 1:dim(y)[2]){
        gneglogpost = gneglogpost + (dim(CGGP$design)[1]+dim(Xs)[1])*dsigma2_hat[,i] / sigma2_hat[i]
      }
