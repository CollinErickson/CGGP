...
if (F) {
  ...
  p3 <- ggplot(data=ecdf2 %>% filter(Package %in% c("CGGP","MRFA","laGP","BASS"), selection.method %in% c("NA","UCB"),correlation %in% c("PowerExp","NA"), f!='beambending') %>% mutate(pointsize=1+(Package=="CGGP")), mapping=aes(n, runtime, color=Package, shape=Package, group=Package, linetype=Package, size=Package)) + geom_point(mapping=aes(size=Package))+ scale_size_manual(values=c(2,4,2,2), name='Package') + facet_wrap(. ~ f, scales="free") + scale_y_log10() + scale_x_log10() + scale_shape_manual(values=c(15,17,16,18)) + stat_smooth(se=F, alpha=.6, geom='line') +scale_linetype_manual(values=c(2,1,3,4), name='Package') + scale_color_manual(values=c("#F8766D", "#00BFC4","#7CAE00", "#C77CFF"), name="Package"); p3
  # Mean smoothing
  p1 <- ggplot(data=ecdf2 %>% filter(Package %in% c("CGGP","MRFA","laGP","BASS"), selection.method %in% c("NA","UCB"),correlation %in% c("PowerExp","NA"), f!='beambending') %>% mutate(pointsize=1+(Package=="CGGP")), mapping=aes(n, RMSE, color=Package, shape=Package, group=Package, linetype=Package, size=Package)) + geom_point(mapping=aes(size=Package))+ scale_size_manual(values=c(2,4,2,2), name='Package') + facet_wrap(. ~ f, scales="free") + scale_y_log10() + scale_x_log10() + scale_shape_manual(values=c(15,17,16,18)) + scale_linetype_manual(values=c(2,1,3,4), name='Package') + scale_color_manual(values=c("#F8766D", "#00BFC4","#7CAE00", "#C77CFF"), name="Package")+ stat_summary(fun.y=mean, geom="line", alpha=.6); p1
  p2 <- ggplot(data=ecdf2 %>% filter(Package %in% c("CGGP","laGP","BASS"), selection.method %in% c("NA","UCB"),correlation %in% c("PowerExp","NA"), f!='beambending') %>% mutate(pointsize=1+(Package=="CGGP")), mapping=aes(n, CRPscore, color=Package, shape=Package, group=Package, linetype=Package, size=Package)) + geom_point(mapping=aes(size=Package))+ scale_size_manual(values=c(2,4,2,2), name='Package') + facet_wrap(. ~ f, scales="free") + scale_y_log10() + scale_x_log10() + scale_shape_manual(values=c(15,17,16,18)) + scale_linetype_manual(values=c(2,1,3,4), name='Package') + scale_color_manual(values=c("#F8766D", "#00BFC4","#7CAE00", "#C77CFF"), name="Package") + stat_summary(fun.y=mean, geom="line", alpha=.6); p2
  p3 <- ggplot(data=ecdf2 %>% filter(Package %in% c("CGGP","MRFA","laGP","BASS"), selection.method %in% c("NA","UCB"),correlation %in% c("PowerExp","NA"), f!='beambending') %>% mutate(pointsize=1+(Package=="CGGP")), mapping=aes(n, runtime, color=Package, shape=Package, group=Package, linetype=Package, size=Package)) + geom_point(mapping=aes(size=Package))+ scale_size_manual(values=c(2,4,2,2), name='Package') + facet_wrap(. ~ f, scales="free") + scale_y_log10() + scale_x_log10() + scale_shape_manual(values=c(15,17,16,18)) +scale_linetype_manual(values=c(2,1,3,4), name='Package') + scale_color_manual(values=c("#F8766D", "#00BFC4","#7CAE00", "#C77CFF"), name="Package") + stat_summary(fun.y=mean, geom="line", alpha=.6); p3
  # Mean smoothing, but all normal lines (no dots/dashes). Use true means, not the log scale. For run times, only use means, no dots.
  p1 <- ggplot(data=ecdf2 %>% filter(Package %in% c("CGGP","MRFA","laGP","BASS"), selection.method %in% c("NA","UCB"),correlation %in% c("PowerExp","NA"), f!='beambending') %>% mutate(pointsize=1+(Package=="CGGP")), mapping=aes(n, RMSE, color=Package, shape=Package, group=Package, linetype=Package, size=Package)) + geom_point(mapping=aes(size=Package))+ scale_size_manual(values=c(2,4,2,2), name='Package') + facet_wrap(. ~ f, scales="free") + scale_y_log10() + scale_x_log10() + scale_shape_manual(values=c(15,17,16,18)) + scale_linetype_manual(values=c(1,1,1,1), name='Package') + scale_color_manual(values=c("#F8766D", "#00BFC4","#7CAE00", "#C77CFF"), name="Package")+ stat_summary(fun.y=mean, geom="line", alpha=.6); p1
  p2 <- ggplot(data=ecdf2 %>% filter(Package %in% c("CGGP","laGP","BASS"), selection.method %in% c("NA","UCB"),correlation %in% c("PowerExp","NA"), f!='beambending') %>% mutate(pointsize=1+(Package=="CGGP")), mapping=aes(n, CRPscore, color=Package, shape=Package, group=Package, linetype=Package, size=Package)) + geom_point(mapping=aes(size=Package))+ scale_size_manual(values=c(2,4,2,2), name='Package') + facet_wrap(. ~ f, scales="free") + scale_y_log10() + scale_x_log10() + scale_shape_manual(values=c(15,17,16,18)) + scale_linetype_manual(values=c(1,1,1,1), name='Package') + scale_color_manual(values=c("#F8766D", "#00BFC4","#7CAE00", "#C77CFF"), name="Package") + stat_summary(fun.y=mean, geom="line", alpha=.6); p2
  p3 <- ggplot(data=ecdf2 %>% filter(Package %in% c("CGGP","MRFA","laGP","BASS"), selection.method %in% c("NA","UCB"),correlation %in% c("PowerExp","NA"), f!='beambending') %>% mutate(pointsize=1+(Package=="CGGP")), mapping=aes(n, runtime, color=Package, shape=Package, group=Package, linetype=Package, size=Package)) + geom_point(mapping=aes(size=Package))+ scale_size_manual(values=c(2,4,2,2), name='Package') + facet_wrap(. ~ f, scales="free") + scale_y_log10() + scale_x_log10() + scale_shape_manual(values=c(15,17,16,18)) +scale_linetype_manual(values=c(1,1,1,1), name='Package') + scale_color_manual(values=c("#F8766D", "#00BFC4","#7CAE00", "#C77CFF"), name="Package") + stat_summary(fun.y=mean, geom="line", alpha=.6); p3
