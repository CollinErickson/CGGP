...
SGGP_internal_gneglogpost <- function(theta, SGGP, y,..., return_lik=FALSE,ys=NULL,Xs=NULL,HandlingSuppData = "Correct") {
      ...
      for(paralcv in 1:SGGP$numpara){
        ...
        if(is.matrix(dsigma2_hat_supp )){
          if(dim(dsigma2_hat_supp)[1]>1.5){
            dsigma2_hat_supp[(dimlcv-1)*SGGP$numpara+paralcv,] = -colSums(as.matrix(tempvec1*tempvec2))
          }else{
            dsigma2_hat_supp[,(dimlcv-1)*SGGP$numpara+paralcv] = -colSums(as.matrix(tempvec1*tempvec2))
          }
        }else{
  ...
  if(HandlingSuppData == "FullValidation" || 
    ...
    for (dimlcv in 1:SGGP$d) { # Loop over dimensions
      V = SGGP$CorrMat(Xs[,dimlcv], SGGP$xb, theta[(dimlcv-1)*SGGP$numpara+1:SGGP$numpara],return_dCdtheta=TRUE,returnlogs=TRUE)
    ...
    for (dimlcv in 1:SGGP$d) { # Loop over dimensions
      dCs[,,dimlcv] =Cmat1*as.matrix(dCs[,,dimlcv])
      Cmat2 = matrix(rep(GGGG[[dimlcv]], SGGP$numpara ) ,nrow = nrow(GGGG[[dimlcv]]) , byrow = FALSE )
      dGGGG1[[dimlcv]] =Cmat2*as.matrix(dGGGG1[[dimlcv]])
    }
  ...
  if(HandlingSuppData =="Only" || HandlingSuppData == "Mixture"){
    ...
    if(!is.matrix(y)){
      ...
      gneglogpost =gneglogpost+ 0.5*(1/(1-theta)-1/(theta+1))+dim(ys)[2]*dlDet
      for(i in 1:dim(y)[2]){
        gneglogpost = gneglogpost + dim(Xs)[1]*dsigma2_hat[,i] / sigma2_hat[i]
      }
  ...
  if(HandlingSuppData =="Ignore" || HandlingSuppData == "Mixture"){
    ...
    if(!is.matrix(ys)){
      ...
      gneglogpost = gneglogpost+0.5*(1/(1-theta)-1/(theta+1))+dim(y)[2]*dlDet
      for(i in 1:dim(y)[2]){
        gneglogpost = gneglogpost + dim(SGGP$design)[1]*dsigma2_hat[,i] / sigma2_hat[i]
      }
      gneglogpost =  gneglogpost/2
    }
  }
