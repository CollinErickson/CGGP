  ...
  if(!is.null(y) && HandlingSuppData != "Only"){
    ...
    if(!is.null(Xs) && (HandlingSuppData == "FullValidation" || HandlingSuppData == "Correct" || HandlingSuppData == "MarginalValidation" )){
      ...
      for (dimlcv in 1:SGGP$d) { # Loop over dimensions
        ...
        GGGG[[dimlcv]] = SGGP$CorrMat(Xs[,dimlcv], SGGP$xb,theta[(dimlcv-1)*SGGP$numpara+1:SGGP$numpara])
      ...
      Cmat2 = Cmat1
      dGGGG = list(matrix(1,dim(Xs)[1],length(SGGP$xb)*SGGP$numpara),SGGP$d)
      for (dimlcv in 1:SGGP$d) { # Loop over dimensions
      ...
      if(HandlingSuppData =="Correct" || HandlingSuppData =="FullValidation"){
        ...
        for (dimlcv in 1:SGGP$d) {
          Sall = SGGP$CorrMat(SGGP$xb,SGGP$xb,theta[(dimlcv-1)*SGGP$numpara+1:SGGP$numpara], return_dCdtheta = TRUE)
          for (levellcv in 1:max(SGGP$uo[1:SGGP$uoCOUNT,dimlcv])) {
            INDSN = 1:SGGP$sizest[levellcv]
            REEALL =SGGP_internal_postvarmatcalcfaster(GGGG[[dimlcv]],dGGGG[[dimlcv]],Sall,INDSN,SGGP$numpara)
