...
SG = SGcreate(rep(0, d), rep(1, d),501) #create the design.  it has so many entries because i am sloppy
Y = testf(SG$design) #the design is $design, simple enough, right?
logthetaest = logthetaMLE(SG,Y)


source("SGGP_MSEspeed_test.R")
GP = SGGPpred(Xp,SG,Y,logtheta=pmin(logthetaest,1)) #build a full emulator
